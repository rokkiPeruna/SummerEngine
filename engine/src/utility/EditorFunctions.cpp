...
Vec2f ScreenCoordsToNormOpenGLCoords(SEint scr_x, SEint scr_y, Vec2i win_size, Vec3f cam_position)
{
	...
	if (cam_position.z >= 0)
	{
		factor_x = (scr_x < (wsize.x / 2.0f)) ? -1.0f : 1.0f;
		factor_y = (scr_y < (wsize.y / 2.0f)) ? 1.0f : -1.0f; //Y flipped
	}
	else
	{
		factor_x = (scr_x < (wsize.x / 2.0f)) ? -1.0f : 1.0f;
		factor_y = (scr_y < (wsize.y / 2.0f)) ? -1.0f : 1.0f;
	}
	cam_position.z = (cam_position.z == 0.0f) ? 0.1f : cam_position.z; //Avoid multiplying with zero
	return Vec2f
	(
		std::abs(wsize.x / 2.0f - scr_x) * factor_x / (wsize.x / 2.0f) * std::abs(cam_position.z),
		std::abs(wsize.y / 2.0f - scr_y) * factor_y / (wsize.y / 2.0f) * std::abs(cam_position.z)
	);
